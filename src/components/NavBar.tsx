...
export default function Navbar(){
  ...
  return (
    <header className="flex w-full items-center justify-between relative">
      ...
      <button className={`${ open && 'absolute top-[12px] right-[220px]' } px-4 py-2 border-2 border-[#440476] text-[#410372] transition-all hover:font-bold rounded-md lg:hidden text-xl me-[24px]`} onClick={() => setOpen(true)}>
      ...
      <div className={`${open ? 'block absolute top-0 right-0 h-fit md:w-fit justify-self-end bg-[#440770] text-white py-8 text-center space-y-6 px-4 z-10' :'hidden'} lg:flex w-full`}>
        <button className={`absolute top-[16px] right-0 px-4 py-2 text-white transition-all hover:font-bold rounded-md lg:hidden text-xl me-[24px] text-xl text-right`} onClick={() => setOpen(false)}>
        <IoMdCloseCircle />
        </button>
